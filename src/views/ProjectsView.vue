  ...
  <div class="projects-page">
    ...
    <section class="filters-panel">
      ...
      <div class="filters-grid">
        <label>Дата начала от<input v-model="localFilters.startDateFrom" type="date" /></label>
        <label>Дата начала до<input v-model="localFilters.startDateTo" type="date" /></label>
        <label>Дата окончания от<input v-model="localFilters.endDateFrom" type="date" /></label>
        <label>Дата окончания до<input v-model="localFilters.endDateTo" type="date" /></label>
        <label>Приоритет<input v-model.number="localFilters.priority" type="number" min="1" /></label>
        ...
        <label>
          ...
          <select v-model.number="localFilters.projectManagerId">
            <option :value="undefined">Любой</option>
            <option v-for="employee in managerOptions" :key="employee.id" :value="employee.id">{{ employee.lastName }} {{ employee.firstName }}</option>
          </select>
        </label>
    ...
    <div v-else>
      ...
      <ProjectCard v-for="project in projectsStore.projects" :key="project.id" :project="project" @click="goToProject(project.id)" />
    </div>
...
import type { Company, Employee, ProjectFilterParams } from "@/types";
...
const managerOptions = computed<Employee[]>(() => {
  if (employeesStore.employees.length) return employeesStore.employees;
...
<style scoped>
.projects-page { display: flex; flex-direction: column; gap: 1rem; }
.header-row { display: flex; justify-content: space-between; align-items: center; }
.filters-panel { border: 1px solid #e2e2e2; border-radius: 8px; padding: 1rem; }
.filters-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 0.75rem; }
label { display: flex; flex-direction: column; gap: 0.25rem; }
.filter-actions { margin-top: 1rem; display: flex; gap: 0.5rem; }
